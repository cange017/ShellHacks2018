      ...
      <div>
        {correctChat &&
          ...
          correctChat.messages.map((message, i) => (
            <div key={i} className={message.from === ME ? 'me' : 'them'}>{makeApiRequest(endpoints.translate, {textToTranslate: message.content}, "GET", false).data.translations[0].translatedText}</div>
          ))}
      </div>
    );
  }
